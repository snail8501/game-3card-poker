...
export default function GameRoom({ gameId, token }) {
  ...
  const gameServer = useGameRoom(gameId, token)
  ...
  useEffect(() => {
    ...
    if(state.msgType === 0) {
      ...
      if(state.event) {
        const { event } = state
        if(event.type === 0) {
          console.log(`事件${event.type} 玩家 #${event.userId} 加入游戏(如果消息通知，则表示广播)->用户状态`, event)
        } else if(event.type === 1) {
          console.log(`事件${event.type} 玩家 #${event.userId} 准备游戏->用户状态`, event)
        } else if(event.type === 2) {
          console.log(`事件${event.type} 玩家 #${event.userId} 游戏中->用户状态`, event)
        } else if(event.type === 3) {
          console.log(`事件${event.type} 玩家 #${event.userId} 弃牌->用户状态`, event)
        } else if(event.type === 4) {
          console.log(`事件${event.type} 玩家 #${event.userId} PK输家`, event)
          stateFetcher('local:gameCountdown', {}).then(gameCountdownMutate)
          stateFetcher('local:gamePK', event).then(gamePKMutate)
        } else if(event.type === 5) {
          console.log(`事件${event.type} 玩家 #${event.userId} 最终赢家->用户状态`, event)
          stateFetcher('local:gameCountdown', {}).then(gameCountdownMutate)
          // await aleoFetcher('execute', {
          ...
          console.log(`事件${event.type} 玩家 #${event.userId} 看牌`, event)
        } else if(event.type === 7) {
          console.log(`事件${event.type} 玩家 #${event.userId} 跟注/加注`, event, tabledChips)
          ...
          console.log(`事件${event.type} 玩家 #${event.userId} 用户设置自动下注`, event)
        } else if(event.type === 9) {
          console.log(`事件${event.type} 玩家 #${event.userId} 当前活动用户`, event)
          ...
          console.log(`事件${event.type} 玩家 #${event.userId} 错误请求`, event)
        } else if(event.type === 11) {
          console.log(`事件${event.type} 玩家 #${event.userId} 第${state.room.currRound}轮结束`, event)
          stateFetcher('local:allowPK', false).then(allowPKMutate)
          ...
          if(cUser && cUser?.amount) {
            ...
            }).then((result) => { console.log(result, '游戏过程上链结果') })
          }
        }
      }
